...
# canvas = cv2.morphologyEx(canvas, cv2.MORPH_CLOSE, np.ones((50,50), np.uint8))
# cv2.imshow("canvas opened", canvas)
# canvas = cv2.morphologyEx(canvas, cv2.MORPH_OPEN, np.ones((5,5), np.uint8))
# cv2.imshow("canvas after morph", canvas)
