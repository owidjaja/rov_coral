...
def eyedrop(img):
    ...
    cv2.createTrackbar("hue_track", "Trackbar_Window", 8, 180//2, cb_nothing)
    ...
    cv2.createTrackbar("sat_track", "Trackbar_Window", 20, 255//2, cb_nothing)
...
if __name__ == "__main__":
    ...
    old_cropped, _ = align_base(old_src, approx_height_ratio=4, resize=True)
    new_cropped, crop_tuple = align_base(new_src, approx_height_ratio=5, resize=True)

    # cv2.imshow("old_cropped", old_cropped)
    cv2.imshow("new_ROI", new_cropped)
    ...
    cv2.imshow("new_BGR", new_cropped)
    ...
    print("ops", old_pink.shape)
    canvas = get_diff(old_pink, old_white, new_pink, new_white, max_dist=16, close_ksize=3)
    # cv2.rectangle(canvas, (260,80), (330,180), (0,0,0), -1)
    cv2.imshow("Canvas", auto_resize(canvas, 360))
    new_drawn = draw_diff(canvas, new_cropped, min_area=400)

    # lower_x, lower_y, upper_x, upper_y = crop_tuple
    # out_new = new_src.copy()
    # print("out_new.shape", out_new.shape)
    # print("out_new[lower_y:upper_y, lower_x:upper_x]", out_new[lower_y:upper_y, lower_x:upper_x].shape)
    # print("new_drawn.shape", new_drawn.shape)
    ...
    # cv2.imshow("old_src", auto_resize(old_src))
    # cv2.imshow("old_src", auto_resize(cv2.imread("old_src.JPG"), target_width=700))
    cv2.imshow("OLD_CORAL", auto_resize(old_cropped,target_width=360))
    cv2.imshow("NEW_DRAWN", auto_resize(new_drawn, target_width=360))
